...
cache_file="$HOME/.cache/rofi_emoji_history"
msg='** note ** 👀 Seçince otomatik yapıştırır'
...
# 1. DATA kısmını bul ve emojileri çek (Hata payını sıfıra indirdik)
ALL_EMOJIS=$(awk '/^# # DATA # #$/ {p=1; next} p' "$0")

# 2. Frekans Takibi: Geçmişteki en popülerleri başa al
if [ -s "$cache_file" ]; then
    # Geçmişi say, en çok kullanılanları bul
    TOP_EMOJIS=$(sort "$cache_file" | uniq -c | sort -rn | awk '{print $2}')
    # Listeyi birleştir ve mükerrerleri (tekrarları) sil
    FINAL_LIST=$(echo -e "${TOP_EMOJIS}\n${ALL_EMOJIS}" | awk '!x[$1]++')
else
    FINAL_LIST="$ALL_EMOJIS"
fi

# 3. Rofi Menüsünü Aç
SELECTED=$(echo "$FINAL_LIST" | rofi -i -dmenu -mesg "$msg" -config "$rofi_theme" | awk '{print $1}')

# Eğer seçim yapılmadıysa çık
[ -z "$SELECTED" ] && exit

# 4. Seçimi kaydet ve yapıştır
